...
// Reconcile â€” cek 10 menit terakhir, fix berdasarkan database
...
func (h *ReconcileHandler) Reconcile(w http.ResponseWriter, r *http.Request) {
	...
	// 1. Provider success tapi member belum success â†’ settle member success
	...
	// 2. Member success tapi saldo member belum dipotong (debit < biaya_perkiraan) â†’ potong
	...
	// 3. Member success tapi saldo provider belum dipotong â†’ potong
...
// Provider success tapi member masih pending â†’ settle member success
...
HAVING COALESCE(SUM(CASE WHEN md.arah = 'DEBIT' THEN md.jumlah ELSE 0 END), 0)
	...
	for rows.Next() {
		...
		// Debit selisih â€” atomic: update saldo + insert mutasi dalam satu transaksi
