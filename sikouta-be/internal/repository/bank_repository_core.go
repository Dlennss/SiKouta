...
const (
	...
	SiKoutaDepositBankName      = "BNI"
	SiKoutaDepositAccountNumber = "1955637480"
	SiKoutaDepositAccountHolder = "M Sansan Irfanda"
)
...
// EnsureSiKoutaDepositBank keeps the public deposit account available after
// deployment, including installations where SQL migrations are applied later.
func (r *BankRepository) EnsureSiKoutaDepositBank(ctx context.Context) error {
	if r == nil || r.db == nil {
...
`, SiKoutaDepositAccountNumber, SiKoutaDepositBankName, SiKoutaDepositAccountHolder)
	if err != nil {
		return err
	}
...
SET nomor_rekening = EXCLUDED.nomor_rekening,
    ...
    admin_staff_only = false,
    diubah_pada = now()
`, SiKoutaDepositBankName, SiKoutaDepositAccountNumber, SiKoutaDepositAccountHolder)
	return err
}
