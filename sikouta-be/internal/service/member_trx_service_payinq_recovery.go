...
func (h *MemberTrxService) RecoverPendingRef(ctx context.Context, memberID int64, refID string) (string, int64, error) {
	...
	// Cek apakah ada provider yang sudah sukses â€” kalau ada, settle member jika masih pending
	...
	// - Provider lain (kecuali SMB): PAY ulang dengan refid sama â†’ respon idempoten "sdh pernah...Sukses/Gagal"
	for _, att := range triedAttempts {
		if att == nil {
			continue
		}
		...
		// Cek respon row baru dulu â€” mungkin "sdh pernah...Sukses" meski callErr == nil
		if newRowID > 0 {
...
func (h *MemberTrxService) ResendPendingProviderRow(ctx context.Context, providerTrxID int64) (string, int64, error) {
	...
	if memberStatus == "failed" {
		// Reopen member ke pending â€” admin sengaja kirim ulang
		ketPending, _ := helper.SafeMemberKeterangan("pending", "admin resend: reopen dari failed")
		_ = h.MemberRepo.UpdateTransaksiMemberStatus(ctx, trx.ID, "pending", ketPending, 0)
		h.logf("ADMIN RESEND reopen failedâ†’pending refid=%s trx_id=%d", trx.RefID, trx.ID)
		// Refresh trx
