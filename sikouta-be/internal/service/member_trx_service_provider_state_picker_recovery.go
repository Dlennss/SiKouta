...
import (
	...
	"sikouta/db"
	trxmemberdto "sikouta/internal/dto/trx_member"
...
func (h *MemberTrxService) tryStatusPayFallback(ctx context.Context, trx *repository.TrxMemberFull, tried map[string]bool) (string, int64, error) {
	...
	// untuk refid ini â€” kalau ada, jangan fallback
	if h.JPRepo != nil {
		...
		if lErr == nil {
			...
			for _, att := range allAttempts {
				...
				// Bukan success, bukan definitelyFailed, bukan pending â†’ status tidak jelas, block
			...
			h.logf("GUARD tryStatusPayFallback refid=%s PASSED â€” all attempts definitely failed, proceeding with fallback", trx.RefID)
		} else {
			h.logf("GUARD tryStatusPayFallback refid=%s ListByRefID error=%v â€” BLOCKING fallback (safety)", trx.RefID, lErr)
			return "", 0, fmt.Errorf("gagal cek attempt existing refid=%s: %w", trx.RefID, lErr)
		}
	} else {
		h.logf("GUARD tryStatusPayFallback refid=%s JPRepo=nil â€” BLOCKING fallback (safety)", trx.RefID)
