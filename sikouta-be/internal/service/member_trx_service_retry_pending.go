...
// RetryPendingTransactions â€” cari transaksi pending > 5 menit dan recheck status provider.
...
func (h *MemberTrxService) retryPendingRows(ctx context.Context, rows []*repository.TrxMemberFull) int {
	...
	for _, trx := range rows {
		...
		// Classify: sukses â†’ settle member, gagal â†’ settle member failed
		...
		// status == "pending" â†’ biarkan, coba lagi di cycle berikutnya
