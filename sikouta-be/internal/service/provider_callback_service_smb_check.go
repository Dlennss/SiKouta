...
func (s *ProviderCallbackService) processSMBCheckStage(ctx context.Context, row *repository.ProviderTrxRefRow, trx *repository.CallbackTrxMemberFull, data *smbCallbackData) (bool, int, map[string]any) {
	...
	if trx.Status == "success" || trx.Status == "failed" {
		helper.AppendAlreadyFinalLog("callback already_final provider=smb stage=check refid=%s trx_member_id=%d status=%s â€” jangan bayar", data.refid, trx.ID, trx.Status)
	...
	if smb.LooksLikeImmediateReject(data.msg) {
		...
		if latestRow != nil && strings.EqualFold(strings.TrimSpace(latestRow.Status), "failed") {
			helper.AppendProviderServiceLog("provider_callback_service.log", "SMB DISPATCH PAY dibatalkan â€” provider row sudah failed refid=%s", data.refid)
		...
		if latestTrx != nil && (latestTrx.Status == "success" || latestTrx.Status == "failed") {
			helper.AppendProviderServiceLog("provider_callback_service.log", "SMB DISPATCH PAY dibatalkan â€” member sudah %s refid=%s", latestTrx.Status, data.refid)
		...
		if isBank {
			helper.AppendProviderServiceLog("provider_callback_service.log",
				"SMB PAY bank dispatched â€” pending tunggu callback BYR refid=%s status_raw=%s", data.refid, data.statusRaw)
