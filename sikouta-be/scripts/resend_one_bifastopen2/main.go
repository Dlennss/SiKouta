...
func main() {
	if len(os.Args) < 2 {
		log.Fatal("usage: resend_one_bifastopen2 <refid>")
	}
	...
	var attempt int
	if err := db.QueryRow(`SELECT COALESCE(MAX(percobaan),0)+1 FROM transaksi_provider WHERE transaksi_member_id=$1 AND provider='smb' AND produk_provider_map_id=$2`, t.TrxID, t.BackupMapID).Scan(&attempt); err != nil {
		log.Fatal(err)
	}
	...
	if raw := strings.TrimSpace(os.Getenv("SMB_TIMEOUT")); raw != "" {
	...
	if resp.HTTPStatus == 200 {
		status = helper.ProviderResponseStatusString("smb", ptrString(resp.RC), ptrString(msg))
	}
