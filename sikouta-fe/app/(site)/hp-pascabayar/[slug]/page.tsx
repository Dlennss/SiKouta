...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return buildPageMetadata({
      title: "HP Pascabayar | SiKouta",
      description: "Cek dan bayar tagihan HP pascabayar di SiKouta.",
      path: "/hp-pascabayar",
    });
  }

  return buildPageMetadata({
    title: `${toTitleCase(brand.nama)} | HP Pascabayar SiKouta`,
    description: `Cek dan bayar tagihan ${toTitleCase(brand.nama)} di SiKouta dengan proses yang ringkas.`,
    path: `/hp-pascabayar/${slug}`,
    keywords: [`${String(brand.nama).toLowerCase()} pascabayar`, `bayar ${String(brand.nama).toLowerCase()}`, "hp pascabayar", "sikouta"],
  });
}
...
export default async function HPPascabayarSlugPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `${brandTitle} | HP Pascabayar SiKouta`,
