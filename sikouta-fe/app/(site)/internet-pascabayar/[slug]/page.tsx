...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return buildPageMetadata({
      title: "Internet Pascabayar | SiKouta",
      description: "Cek dan bayar tagihan internet pascabayar di SiKouta.",
      path: "/internet-pascabayar",
    });
  }

  return buildPageMetadata({
    title: `${toTitleCase(brand.nama)} | Internet Pascabayar SiKouta`,
    description: `Cek dan bayar tagihan ${toTitleCase(brand.nama)} di SiKouta dengan proses yang ringkas.`,
    path: `/internet-pascabayar/${slug}`,
    keywords: [`${String(brand.nama).toLowerCase()} pascabayar`, `bayar ${String(brand.nama).toLowerCase()}`, "internet pascabayar", "sikouta"],
  });
}
...
export default async function InternetPascabayarSlugPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `${brandTitle} | Internet Pascabayar SiKouta`,
    description: `Cek dan bayar tagihan ${brandTitle} di SiKouta dengan proses yang ringkas.`,
    path: `/internet-pascabayar/${slug}`,
