...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (universalService) {
    return buildPageMetadata({
      title: `${universalService === "esim-roaming" ? "eSIM & Roaming" : "HP Pascabayar"} | SiKouta`,
      description: "Layanan digital SiKouta.",
      path: `/kategori/${id}`,
    });
  }
  ...
  if (!category) {
    return buildPageMetadata({
      title: "Kategori Produk | SiKouta",
      description: "Lihat kategori produk SiKouta.",
      path: "/kategori",
    });
  }

  return buildPageMetadata({
    title: `${category.nama} | SiKouta`,
    description: `Lihat pilihan brand dan produk ${category.nama} di SiKouta.`,
    path: `/kategori/${id}`,
    keywords: [String(category.nama).toLowerCase(), "produk sikouta", "sikouta"],
  });
}
...
export default async function GuestKategoriPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `${categoryName} | SiKouta`,
    description: `Lihat pilihan brand dan produk ${categoryName} di SiKouta.`,
    path: `/kategori/${id}`,
