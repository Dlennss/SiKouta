...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return buildPageMetadata({
      title: "Tagihan PDAM | SiKouta",
      description: "Cek dan bayar tagihan PDAM di SiKouta.",
      path: "/pdam",
    });
  }

  return buildPageMetadata({
    title: `${brand.nama} | Tagihan PDAM SiKouta`,
    description: `Cek dan bayar tagihan ${brand.nama} di SiKouta dengan proses cepat dan ringkas.`,
    path: `/pdam/${slug}`,
    keywords: [`tagihan ${brand.nama.toLowerCase()}`, `bayar ${brand.nama.toLowerCase()}`, "pdam online", "sikouta"],
  });
}
