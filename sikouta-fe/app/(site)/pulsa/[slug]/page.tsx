...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!pulsaCategory) {
    return buildPageMetadata({
      title: "Pulsa Operator | SiKouta",
      description: "Isi pulsa operator favorit Anda di SiKouta.",
      path: "/pulsa",
    });
  }
  ...
  return buildPageMetadata({
    title: `Isi Pulsa ${brandTitle} Online | SiKouta`,
    description: lowestPrice
      ? `Isi pulsa ${brandTitle} online di SiKouta dengan nominal lengkap. Harga mulai Rp ${lowestPrice.toLocaleString("id-ID")} dan transaksi cepat.`
      : `Isi pulsa ${brandTitle} online di SiKouta dengan nominal lengkap dan transaksi cepat.`,
    path: `/pulsa/${slug}`,
    keywords: [`pulsa ${brand.nama.toLowerCase()}`, `isi pulsa ${brand.nama.toLowerCase()}`, "sikouta"],
...
export default async function GuestPulsaBrandPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `Isi Pulsa ${brandTitle} Online | SiKouta`,
    description: `Isi pulsa ${brandTitle} online di SiKouta dengan nominal lengkap dan transaksi cepat.`,
    path: `/pulsa/${slug}`,
  ...
  const faqJsonLd = buildFaqJsonLd([
    {
      question: `Apakah nominal pulsa ${brandTitle} di SiKouta lengkap?`,
      answer: `Ya. SiKouta menampilkan pilihan nominal pulsa ${brandTitle} yang aktif agar pembeli bisa memilih sesuai kebutuhan.`,
    },
    {
      question: `Bagaimana cara beli pulsa ${brandTitle} di SiKouta?`,
      answer: `Pilih nominal pulsa ${brandTitle}, masukkan nomor tujuan, lalu lanjutkan pembayaran sesuai metode yang tersedia.`,
    },
    {
      question: `Apakah pembelian pulsa ${brandTitle} di SiKouta bisa untuk calon member dan member?`,
      answer: `Bisa. Halaman ini bisa dipakai pembeli umum, dan member SiKouta juga bisa memanfaatkan pilihan produk yang sama untuk transaksi harian.`,
    },
  ]);
