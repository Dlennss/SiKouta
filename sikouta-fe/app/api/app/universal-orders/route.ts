...
function apiBase() {
  return (process.env.NEXT_PUBLIC_API_BASE || process.env.API_BASE || "http://127.0.0.1:8091").replace(/\/+$/, "");
}
...
export async function POST(req: Request) {
  ...
  const service = cleanText(payload.service, "Layanan SiKouta");
  const destination = cleanText(payload.destination);
  const provider = cleanText(payload.provider, "SiKouta");
