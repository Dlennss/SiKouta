...
function getPaymentHelper(methodId: string, applicationId: number) {
  if (methodId === "qris") {
    return {
      ...
      desc: "Scan QRIS dari kanal SiKouta, lalu upload bukti pembayaran.",
      rows: [
...
function getApplicantLabel(item: AgentCreditApplication) {
  ...
  return {
    name: String(data.agent_name || data.nama_lengkap || "Agent SiKouta"),
...
export function UserCreditBillPayLaterContent({ bills }: Props) {
  ...
  async function copyPaymentValue() {
    if (!paymentHelper.value || paymentHelper.value === "Saldo akun SiKouta") return;
